...
const LoginPembeli: React.FC = () => {
  ...
  useEffect(() => {
    document.title = 'Login Pembeli | Vocallet';
  }, []);
  ...
  const loginDenganGoogle = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      try {
        ...
        navigate('/pedagang-sekitar'); // Adjust if buyer has a different dashboard
      } catch (err: any) {
    ...
    <div className="min-h-screen bg-[#f0f7ff] text-gray-900 flex flex-col items-center justify-center p-6 relative antialiased w-full">
      ...
      <header className="w-full py-4 px-6 flex items-center justify-center absolute top-0 left-0">
        <div className="absolute inset-x-0 top-0 py-4 flex justify-center pointer-events-none">
          <span className="font-extrabold text-lg md:text-xl text-blue-600 tracking-wide">
            Vocallet
          </span>
        </div>
      </header>
      ...
      >
        ...
        <div className="w-full bg-white p-6 md:p-8 rounded-3xl shadow-sm border border-blue-50">
          ...
          {/* Heading */}
          <h1 className="text-2xl font-extrabold text-gray-800 tracking-tight text-left mb-1">Akses Akun Pembeli</h1>
          <p className="text-sm text-gray-500 text-left mb-6">Silakan masuk untuk mulai berbelanja.</p>
          ...
          <form onSubmit={handleSubmit} className="w-full flex flex-col">
            ...
            <div className="flex items-center w-full mb-6">
              <input
                ...
                className="w-3 h-3 scale-[1.2] origin-left text-blue-600 border-gray-300 rounded focus:ring-blue-600 accent-blue-600 cursor-pointer"
              />
            ...
            >
              {loading ? 'Memproses...' : 'Masuk Akun Pembeli'}
            </button>
          </form>
